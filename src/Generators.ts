export class Generators {
  ...
  static right = [
    ...
    // Gergely Mentsik - a famous software engineer in health care
    'gergely',
    ...
    // a famous software engineer in health care
    'mentsik',
  ...
  public static randomName(sep = '_') {
    let randnum = (n) => Math.floor(Math.random() * (n + 1));
    return Generators.left[randnum(Generators.left.length)] + sep + Generators.right[randnum(Generators.right.length)];
  }


  public static randomPhone() {
    return '0664' + Math.floor(Math.random() * Math.floor(10000000)).toString();
  }

  public static randomMail() {
    return Generators.randomName('.') + '@random.com';
  }
}
